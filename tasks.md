...
- Latency follow-up: fresh no-tool agent turns remained 5–8 seconds on Mac while the Grasshopper hook took milliseconds. A bounded code lookup was about twice as fast at low versus high Codex reasoning effort; quality on harder work is untested. A stalled synthetic backend exposed a five-second startup wait; this checkout now limits it to two seconds. `go test ./...`, `go vet ./...`, and `go test -race ./internal/goclient` pass. The limit is not yet in a published package or installed client.
