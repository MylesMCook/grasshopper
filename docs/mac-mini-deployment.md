...
The Mac service and Beelink HTTPS MCP read/write are verified. CLI checks against this persistent endpoint are recorded in [acceptance evidence](memory-acceptance.md); remaining fallbacks and the final package are still being checked. A reboot, Bitwarden sync from another device, and a real-profile restore are untested.
