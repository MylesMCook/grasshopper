...
**Earlier 24 September view update:** Clean source commit `5e19b96` added cross-device browsing. The old binary and plist were copied to a private task-local rollback directory before restarting only `com.myles.grasshopper`. Local and Tailnet health, anonymous rejection, the view API, and a historical MCP read passed afterward. The database and route were unchanged. The live view remains empty because all synthetic pilot records are archived; no personal profile was seeded. See [verification](memory-acceptance.md). Check [releases](https://github.com/MylesMCook/grasshopper/releases) for the current portable package.

**Annotation compatibility pilot:** The Mac mini visualizer now has a compact layout and an opt-in two-hash CSP allowance for the styles injected by Codex app build 26.917.71314. The exact hashes are in the Grasshopper launchd arguments; the server's default policy remains strict and no `unsafe-inline` source is allowed. A task-local in-app-browser test selected the heading successfully. Recompute and retest the hashes after a Codex update; remove the `--visualizer-style-hashes` argument to return to the default policy. The prior binary and plist are in the private visualizer task folder. The approved restart preserved the existing loopback listener, private route, and memory counts; local/private health and auth checks passed. The live-tab selection check is recorded in [verification](memory-acceptance.md).
...
The Mac service and Beelink HTTPS MCP read/write are verified. CLI checks against this persistent endpoint and their limits are recorded in [acceptance evidence](memory-acceptance.md). A reboot, Bitwarden sync from another device, and a real-profile restore are untested.
