...
**Current state:** Grasshopper has a fresh private Mac mini service and encrypted off-host backups. Its Tailnet HTTPS endpoint passed authenticated MCP read/write from Beelink; no legacy database was migrated. Synthetic backend, package, and fresh CLI context checks passed on Mac, Work HP Windows, and Beelink Ubuntu. The persistent endpoint has not yet passed all three CLI harnesses. Desktop checks are user-led if an issue appears.
...
| Private proxy | The published 2.0.0 server returned `403` on Beelink MCP POST because the SDK rejected the private HTTPS Host. A regression test reproduced it. A checked 2.0.1 candidate archive is now installed: it accepts one configured exact proxy Host, rejects others and mismatched Origins, and kept local MCP working. Beelink repeated authenticated `context`, `store`, `get`, and `archive` on the extracted archive binary. | Publish the final 2.0.1 archives after the scope-guidance fix. |

The active Mac binary is from the checked 2.0.1 candidate archive at `0d4bb53`; packaging of the final scope-guidance change is in progress. Detailed host setup and rollback are in [Mac mini private service](mac-mini-deployment.md). These checks do not establish public accessibility; the route uses Tailscale Serve, not Funnel.

**Live CLI reads:** Mac Claude Code 2.1.280 and Cursor Agent 2026.09.23 received project record 4 revision 1 (`lilac compass`) from the persistent service before tool use. Beelink Claude Code 2.1.281 and Cursor Agent received the same record through their startup hooks; Claude also fetched it with an explicit MCP read. Work HP Claude Code 2.1.280 received it in a first turn. Direct Beelink and Work HP hooks resolved the same stable project ID. A fresh Mac Codex CLI 0.156.1 initially sent a folder path, then `Darwin`, and missed context. With corrected shared policy and MCP schema, a fresh Codex CLI turn read the record from a task-local shadow server. A live Codex retry against the final installed build is still open. Work HP Codex CLI 0.155.1 stalled past its 120-second model-turn cap. Cursor Agent CLI 2026.09.23 timed out at 90 seconds; its project MCP source still needed approval. Both direct startup hooks loaded the record, but neither model turn proved receipt. Cursor's headless `--mode ask` denied MCP; normal `--print` with a project read allowlist and explicit MCP approval worked on Mac. These observations are CLI evidence, not Desktop proof.
