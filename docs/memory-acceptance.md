...
| Private proxy | The published 2.0.0 server returned `403` on Beelink MCP POST because the SDK rejected the private HTTPS Host. A regression test reproduced it. A checked 2.0.1 candidate archive accepts one configured exact proxy Host, rejects others and mismatched Origins, and kept local MCP working. Beelink repeated authenticated `context`, `store`, `get`, and `archive` on the extracted archive binary. | Full CLI compatibility is tracked below. |

The proxy check used the 2.0.1 candidate archive at `0d4bb53`. Detailed host setup and rollback are in [Mac mini private service](mac-mini-deployment.md). These checks do not establish public accessibility; the route uses Tailscale Serve, not Funnel.
