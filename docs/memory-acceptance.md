...
**2.1.0 release gate:** [CI on `47507dc`](https://github.com/MylesMCook/grasshopper/actions/runs/36093549038) passed tests, vet, and native builds on Mac, Linux, and Windows; Mac/Linux race checks passed. Six refreshed archives passed internal SHA-256 checks (25 entries per client, 70 per server). The server binaries in those archives match the native CI artifacts. The draft is not published; its older assets still need replacing.
