...
| Private proxy | The published 2.0.0 server returned `403` on Beelink MCP POST because the SDK rejected the private HTTPS Host. A regression test reproduced it. A source-built hotfix now accepts one configured exact proxy Host, rejects other Hosts and mismatched Origins, and kept local MCP working. Beelink read/write passed after the fix. | Publish and install 2.0.1 archives, then repeat the remote check on the released binary. |

The active Mac binary is the tested source hotfix while 2.0.1 packaging is in progress. Detailed host setup and rollback are in [Mac mini private service](mac-mini-deployment.md). These checks do not establish public accessibility; the route uses Tailscale Serve, not Funnel.
