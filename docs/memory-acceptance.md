...
| Cursor Agent CLI 2026.09.23-86fc751, Mac | A fresh headless session with a project-local read-tool allowlist used the Go client to get current and prior revisions, explain “concise” → “direct,” and cite provenance. | First run rejected `get` despite `--approve-mcps` and looped; the allowlist fixed that run. Work HP CLI needs the same check. No Desktop inference. |
...
**Next check:** repeat the state-change probe through Work HP Cursor and Claude Code CLIs, test CLI resume/compaction and installation/removal, then rehearse persistent startup and rollback on an approved private host. The user will test desktop apps if an issue appears. Do not release or migrate live data on the strength of these synthetic tests alone.
