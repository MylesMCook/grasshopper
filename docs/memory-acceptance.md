...
**2.1.0 draft gate:** [CI on `a223077`](https://github.com/MylesMCook/grasshopper/actions/runs/36090493139) passed Mac, Linux, and Windows tests, vet, and native builds; Mac and Linux race checks passed. Six refreshed archives built from that code passed internal SHA-256 checks (25 entries per client, 75 per server), and their external checksums were generated. The extracted final Mac server passed synthetic authentication and viewer checks. The final Linux server archive ran on Beelink as described below. Windows passed CI, but its packaged server has not run on a physical Windows host. The draft is not published; uploaded draft assets still need replacing.
