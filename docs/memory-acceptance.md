...
**Release evidence:** [Grasshopper 2.2.0](https://github.com/MylesMCook/grasshopper/releases/tag/v2.2.0) is public. The private Mac mini service still runs its earlier binary. Encrypted off-host backup and restore passed earlier. Fresh Codex and Cursor CLI turns on Mac mini and Beelink read the same live project memory in earlier checks. No legacy data was migrated or personal profile seeded.

## Marketplace pilot, 25 September

The `marketplace-candidate` branch packages one stateless connector per OS for Codex and Cursor. Its archive passed checksum and manifest checks. All tests below used isolated profiles or scratch workspaces and synthetic records. No live memory or persistent service changed.

| Machine and client | Observed behavior | Open limit |
|---|---|---|
| Mac mini, Codex CLI 0.156.1 | Native Git install, hook trust, fresh first-turn context before tools, Cursor's correction readback, bounded outage (4.5s), and removal passed. | Desktop is separate evidence. |
| Mac mini, Cursor Agent CLI 2026.09.23 | Native Git install and MCP approval passed. Fresh first-turn context arrived; registered MCP `store` corrected synthetic record 1 from revision 1 to 2, and `get` read both revisions. Outage finished in 8.6s without a save claim. Native update and removal passed; the update used the same version. | A changed-version update is untested. |
| Beelink Ubuntu x64, Cursor Agent CLI 2026.09.23 | Extracted connector and project MCP passed authentication. A fresh `--plugin-dir` turn received global record 1 and root AGENTS.md before tools. Outage finished in 7.2s without a save claim. Native Git marketplace install then enabled the Linux plugin in an isolated workspace; a fresh turn found startup memory absent, called `context` once, and read synthetic record 1 revision 1 (`amber grass`) with root marker `LINUX-SAGE-504`. | A native Git-install MCP write was not tested there. |
| Beelink Ubuntu x64, Codex CLI 0.156.1 | Native Git install, same-version marketplace upgrade, plugin removal, and marketplace removal passed in an isolated profile. | Candidate first-turn model behavior was not tested there. |
| Work HP Windows x64, Codex CLI 0.155.1 | Native Git install, hook trust, fresh first-turn context before tools, historical readback, bounded outage (15s), and removal passed in scratch. | Desktop is separate evidence. |
| Work HP Windows x64, Cursor Agent CLI 2026.09.23 | Native Git install and MCP approval passed. A project-level MCP entry listed five ready tools but was absent inside fresh Agent sessions. Moving only that entry to a task-local **user-level** Cursor config fixed it: a fresh turn called `context`, then a separate turn used registered MCP `store` to correct synthetic record 1 from revision 2 to 3 and `get` to read both revisions. The bundled `setup --agents none --cursor-cli` command placed the entry at the user level by default; another fresh turn fetched revision 3. A final **combined** test installed the native plugin in the scratch workspace, connected the user-level MCP entry, and read revision 3 (`teal orchard`) through one fallback `context` call in 34.3s. With the backend stopped, an earlier fresh turn finished in 32.1s and claimed no saved write. | The project-level Cursor CLI path remains broken on this version. The earlier setup read turn took 204s of Cursor model time. |

The final `marketplace` branch is **not published**. Cursor's documented `sessionStart` hook is [fire-and-forget](https://prod.cursor.com/docs/hooks); on the Beelink and combined Work HP runs, the canonical AGENTS.md fallback made one MCP call before work. Cursor [acknowledges the Windows project-approval bug](https://forum.cursor.com/t/cursor-cli-can-see-my-mcp-servers-but-agent-sessions-still-cant-use-them/171696/11) and recommends user-level MCP configuration, which Grasshopper's setup uses by default. Both scratch listeners are stopped and their synthetic tokens removed. No GUI or real work files were touched.

**Changed-version update:** In isolated Mac profiles, Codex upgraded the Git marketplace and replaced its installed connector from 2.2.0 to 2.2.1. Cursor CLI's `marketplace update` reported success but kept commit `5ad8fac`, even after a plugin reinstall. Removing and re-adding the marketplace fetched `a22b3b1`; uninstalling and reinstalling the plugin then loaded 2.2.1. The new connector's `setup --update` changed a scratch MCP entry from the old binary path to the new cache path, and `check` authenticated against a synthetic server. Both scratch plugin registrations, the server, and its token were removed. This is a native CLI lifecycle test, not an IDE update test. Cursor [documents the remove/re-add workaround](https://forum.cursor.com/t/cli-added-git-marketplace-never-reindexes-plugin-marketplace-update-reports-0-plugins-indexed-and-cache-stays-pinned-to-the-first-commit/169587/7).
...
- Three client and three server archives match the native CI binaries. All 288 internal SHA-256 checks passed (26 per client, 70 per server). GitHub's seven asset digests match the staged archives and external `SHA256SUMS`.
- The extracted final Mac package passed private quickstart, anonymous health 401, authenticated client setup and check, checked backup, and offline refusal without config writes. The Linux archive passed the same checks on Beelink Ubuntu x64 using a task-local loopback server. The copied archives matched source hashes, and the test port was closed. The 2.2.0 Windows archive passed native CI and archive checks but was not run on a physical Windows host.
- The public `/setup/` page now links to all six live 2.2.0 archives; each download returned HTTP 200. A Chromium check found all six links, no horizontal overflow at 390px, a working theme switch, and no console errors. Public `/mcp` and `/visualizer/` returned 404. Wrangler deployed version `807de18c-62e6-42c2-a086-9ac5b0833b69`; the prior version was `757304d5-06c1-470d-b71a-11e31ee73ac5` for rollback.
- New installer model turns, Desktop/IDE first-turn behavior, physical Windows 2.2.0, and persistent live-service update remain untested. Native Codex hook trust and Cursor MCP approval were not bypassed.

## Simpler setup and update pilot, Mac mini, 25 September

These were **pre-release 2.2.0 candidate checks** against a task-local server and synthetic data. The live private service and real agent settings were untouched.

- The extracted Mac client archive passed all internal checksums. `setup` authenticated before writing config, kept the token in its existing file, and installed task-local Cursor MCP and hook entries. With the backend stopped, a fresh setup failed in under a second and created neither client nor Cursor config.
- Codex CLI 0.156.1 and Claude Code CLI 2.1.280 used isolated profiles. Each installed the extracted 2.2.0 plugin, updated to a separate 2.2.1 test package, and reported the new version. Both plugin and marketplace registrations were removed afterward. Cursor wiring moved to the new binary and was removed without touching unrelated settings. One command also configured all three against the same synthetic server. These are **native CLI installation checks**, not new model-turn, Desktop, or IDE checks.
- A deliberately invalid Codex marketplace made `setup --update` fail. The previous 2.2.1 plugin remained installed and the client config digest was unchanged after rollback. Unit tests cover the same failure path and reject unknown agents, conflicting Cursor entries, and failed authentication before writes.
- The new server-backup command made a consistent snapshot while the prior task-local server was running. A new server archive passed 70 internal checksums, reopened the same database, saved synthetic record 1 revision 1, and read it after a restart. Its backup passed SQLite `quick_check` with the record and revision intact. The prior 2.1.0 server then reopened and returned that same full record. This tests local binary rollback, not a persistent service-manager rollback or off-host recovery.
- The locally built setup page loaded fonts, styles, and theme without new console errors. At 390px, the document had no horizontal overflow; the theme switch changed its accessible label. The later public-link check is above.
- The final-source Mac client and server archives passed 26 and 70 internal checksums. Their extracted commands reported version 2.2.0, `setup --help` exited successfully, and quickstart returned anonymous health 401. The extracted client authenticated and installed Cursor wiring in an isolated directory; the extracted backup passed SQLite `quick_check`. With that server stopped, `setup` failed in under one second and left no offline config or Cursor directory. Real BGE recall tests passed with the pinned model and runtime. Final Linux and Windows packaging results are above.
