...
**First-run source pilot, 24 September:** A rebuilt Mac archive passed all 75 internal SHA-256 checks. Its extracted server ran `--quickstart` with an isolated data directory and port 18119. It created 0600 token and database files, served `/visualizer/`, returned 401 without a token and 200 with one, and passed SQLite integrity. MCP `store` acknowledged a synthetic record with a ready semantic vector; after stopping and restarting with the same command, MCP `get` returned that record at revision 1. Unit tests reject partial files, linked data directories, and shared-mode directories. This is a Mac synthetic pilot, not a published archive or Linux/Windows physical run.
