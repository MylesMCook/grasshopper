...
fn score_recency_only(hit: &SearchHit) -> f64 {
    ...
    let recency = if days >= 30.0 { 0.0 } else { 0.2 * (1.0 - days / 30.0) };
...
struct TestEntry {
    ...
    entities: Vec<(&'static str, &'static str)>, // (value, kind)
}

fn test_corpus() -> Vec<TestEntry> {
    vec![
        // --- Recent, high-salience knowledge ---
        TestEntry { id: 1, title: "Rust async runtime",
            content: "Tokio is the primary async runtime for Rust. Use spawn_blocking for CPU-bound work.",
            memory_type: "knowledge", created_days_ago: 1.0, last_accessed_days_ago: Some(0.5),
            access_count: 20, salience: 0.9, descriptors: "rust,async,tokio",
            entities: vec![("Tokio", "identifier")] },
        TestEntry { id: 2, title: "SQLite WAL mode",
            content: "SQLite WAL mode allows concurrent reads during writes. Set PRAGMA journal_mode=WAL.",
            memory_type: "knowledge", created_days_ago: 2.0, last_accessed_days_ago: Some(1.0),
            access_count: 15, salience: 0.8, descriptors: "sqlite,database,wal",
            entities: vec![("SQLite", "identifier")] },
        // --- Old, rarely accessed knowledge ---
        TestEntry { id: 3, title: "Python virtualenv",
            content: "Use python -m venv to create virtual environments. Activate with source bin/activate.",
            memory_type: "knowledge", created_days_ago: 90.0, last_accessed_days_ago: Some(60.0),
            access_count: 1, salience: 0.3, descriptors: "python,virtualenv",
            entities: vec![] },
        TestEntry { id: 4, title: "Git rebase workflow",
            content: "Prefer rebase over merge for linear history. Use git rebase -i for interactive cleanup.",
            memory_type: "knowledge", created_days_ago: 45.0, last_accessed_days_ago: Some(30.0),
            access_count: 5, salience: 0.5, descriptors: "git,rebase,workflow",
            entities: vec![] },
        // --- Recent episodes ---
        TestEntry { id: 5, title: "Deployed grasshopper v0.8",
            content: "Today deployed grasshopper v0.8 with entity extraction. Fixed the NLI model loading.",
            memory_type: "episode", created_days_ago: 0.5, last_accessed_days_ago: Some(0.1),
            access_count: 3, salience: 0.7, descriptors: "deploy,grasshopper",
            entities: vec![] },
        TestEntry { id: 6, title: "Debugged memory leak in embedder",
            content: "Yesterday found a memory leak in the ONNX embedder. The model cache was not bounded.",
            memory_type: "episode", created_days_ago: 1.5, last_accessed_days_ago: Some(1.0),
            access_count: 2, salience: 0.6, descriptors: "debug,embedder,memory-leak",
            entities: vec![] },
        // --- Old episodes (should decay fast) ---
        TestEntry { id: 7, title: "Set up CI pipeline",
            content: "Last month set up GitHub Actions CI with cargo test and clippy checks.",
            memory_type: "episode", created_days_ago: 35.0, last_accessed_days_ago: Some(30.0),
            access_count: 1, salience: 0.4, descriptors: "ci,github-actions",
            entities: vec![] },
        TestEntry { id: 8, title: "Fixed DNS resolution",
            content: "2025-01-15 fixed DNS resolution in the Docker container by adding custom resolv.conf.",
            memory_type: "episode", created_days_ago: 60.0, last_accessed_days_ago: None,
            access_count: 0, salience: 0.3, descriptors: "dns,docker",
            entities: vec![] },
        // --- Procedures ---
        TestEntry { id: 9, title: "Deploy grasshopper procedure",
            ...
            memory_type: "procedure", created_days_ago: 10.0, last_accessed_days_ago: Some(2.0),
            access_count: 8, salience: 0.7, descriptors: "deploy,grasshopper,procedure",
            entities: vec![("/usr/local/bin/grasshopper", "file_path")] },
        TestEntry { id: 10, title: "Backup restore procedure",
            ...
            memory_type: "procedure", created_days_ago: 20.0, last_accessed_days_ago: Some(15.0),
            access_count: 3, salience: 0.5, descriptors: "backup,restore,restic",
            entities: vec![("/etc/restic/r2.env", "file_path")] },
        // --- Identity memories (never decay) ---
        TestEntry { id: 11, title: "I prefer simple solutions",
        ...
        TestEntry { id: 13, title: "Common git commands",
            content: "git status, git add, git commit, git push. Basic git workflow for daily use.",
            memory_type: "knowledge", created_days_ago: 50.0, last_accessed_days_ago: Some(0.5),
            access_count: 100, salience: 0.2, descriptors: "git,commands",
            entities: vec![] },
        // --- Low frequency, high salience ---
        TestEntry { id: 14, title: "Security incident response",
            content: "If a security incident occurs: isolate the system, check logs, rotate credentials, notify team.",
            memory_type: "procedure", created_days_ago: 40.0, last_accessed_days_ago: Some(40.0),
        ...
        TestEntry { id: 15, title: "Traefik configuration",
            ...
            entities: vec![("~/services/traefik/", "file_path"), ("/etc/traefik/traefik.yml", "file_path"), ("https://traefik.funnydomainname.com", "url")] },
        TestEntry { id: 16, title: "PocketBase corpus setup",
            ...
            entities: vec![("/home/myles/projects/pocketbase-corpus/", "file_path"), ("PocketBase", "identifier"), ("Cloudflare", "identifier")] },
        // --- Near-duplicates (for dedup / differentiation testing) ---
        TestEntry { id: 17, title: "ONNX model loading",
            content: "ONNX models are loaded via ort crate. Model path: ~/.cache/grasshopper/models/. Use SessionBuilder for configuration.",
            memory_type: "knowledge", created_days_ago: 7.0, last_accessed_days_ago: Some(2.0),
            access_count: 6, salience: 0.6, descriptors: "onnx,models,loading",
            entities: vec![("~/.cache/grasshopper/models/", "file_path"), ("SessionBuilder", "identifier")] },
        TestEntry { id: 18, title: "ONNX runtime configuration",
            content: "ONNX Runtime uses ort 2.0.0-rc.11. Configure with OrtEnvironment. Supports CPU and GPU execution providers.",
            memory_type: "knowledge", created_days_ago: 8.0, last_accessed_days_ago: Some(3.0),
            access_count: 4, salience: 0.5, descriptors: "onnx,runtime,configuration",
            entities: vec![("OrtEnvironment", "identifier")] },
        // --- Varied time + access patterns ---
        TestEntry { id: 19, title: "FTS5 tokenizer setup",
            content: "FTS5 uses porter unicode61 tokenizer. Custom code_expand function for camelCase splitting.",
            memory_type: "knowledge", created_days_ago: 12.0, last_accessed_days_ago: Some(1.0),
            access_count: 8, salience: 0.6, descriptors: "fts5,tokenizer,search",
            entities: vec![("code_expand", "identifier")] },
        TestEntry { id: 20, title: "HNSW index parameters",
            content: "HNSW approximate nearest neighbor search. instant-distance crate. ef_construction=200, M=32 for good recall.",
            memory_type: "knowledge", created_days_ago: 14.0, last_accessed_days_ago: Some(7.0),
        ...
        TestEntry { id: 21, title: "Cloudflare tunnel setup",
            ...
            entities: vec![("/home/myles/.cloudflared/config.yml", "file_path"), ("Cloudflare", "identifier")] },
        TestEntry { id: 22, title: "Systemd service creation",
            ...
            memory_type: "procedure", created_days_ago: 25.0, last_accessed_days_ago: Some(10.0),
            access_count: 6, salience: 0.5, descriptors: "systemd,service,linux",
            entities: vec![("/etc/systemd/system/", "file_path")] },
        TestEntry { id: 23, title: "Cross-encoder reranking",
            content: "BAAI/bge-reranker-base for cross-encoder reranking. fastembed crate. Scores query-passage pairs.",
            memory_type: "knowledge", created_days_ago: 6.0, last_accessed_days_ago: Some(2.0),
            access_count: 5, salience: 0.7, descriptors: "reranking,cross-encoder,search",
            entities: vec![] },
        TestEntry { id: 24, title: "Memory consolidation strategy",
            content: "Consolidate similar memories when count exceeds threshold. Merge content, update salience, archive originals.",
            memory_type: "knowledge", created_days_ago: 9.0, last_accessed_days_ago: Some(4.0),
            access_count: 4, salience: 0.6, descriptors: "consolidation,memory,strategy",
            entities: vec![] },
        // --- Zero-access entries (new, never retrieved) ---
        TestEntry { id: 25, title: "Docker compose networking",
            content: "Docker compose creates default network. Use networks key for custom. Expose vs ports for internal only.",
            memory_type: "knowledge", created_days_ago: 3.0, last_accessed_days_ago: None,
            access_count: 0, salience: 0.5, descriptors: "docker,compose,networking",
            entities: vec![] },
        TestEntry { id: 26, title: "Rust error handling patterns",
            content: "Use anyhow for applications, thiserror for libraries. ? operator for propagation. Context trait for adding context.",
            memory_type: "knowledge", created_days_ago: 4.0, last_accessed_days_ago: None,
            access_count: 0, salience: 0.5, descriptors: "rust,errors,anyhow",
            entities: vec![] },
        // --- Very old, very high access (tests frequency vs decay tradeoff) ---
        TestEntry { id: 27, title: "SSH key management",
            content: "Generate ed25519 keys with ssh-keygen -t ed25519. Add to ssh-agent. Store in ~/.ssh/.",
            memory_type: "knowledge", created_days_ago: 120.0, last_accessed_days_ago: Some(2.0),
            access_count: 50, salience: 0.6, descriptors: "ssh,keys,security",
            entities: vec![("~/.ssh/", "file_path")] },
        // --- Entries with extreme salience values ---
        TestEntry { id: 28, title: "Temporary debug note",
        ...
        TestEntry { id: 30, title: "MCP server deployment",
            content: "Step 1: npm run build in mcp dir. Step 2: sudo systemctl restart mcp-service. Step 3: smoke test with curl.",
            memory_type: "procedure", created_days_ago: 7.0, last_accessed_days_ago: Some(1.0),
            access_count: 10, salience: 0.7, descriptors: "mcp,deploy,procedure",
            entities: vec![] },
        // --- Entity test targets ---
        TestEntry { id: 31, title: "Jina embeddings model",
            content: "Jina Code V2 for embeddings. 768 dimensions. Model at ~/.cache/grasshopper/models/jina-code-v2/.",
            memory_type: "knowledge", created_days_ago: 11.0, last_accessed_days_ago: Some(3.0),
            access_count: 7, salience: 0.6, descriptors: "jina,embeddings,model",
            entities: vec![("~/.cache/grasshopper/models/jina-code-v2/", "file_path")] },
        TestEntry { id: 32, title: "RRF fusion parameters",
            content: "Reciprocal Rank Fusion with k=60. FTS weight 0.4, vector weight 0.6. Merges keyword and semantic results.",
            memory_type: "knowledge", created_days_ago: 13.0, last_accessed_days_ago: Some(5.0),
            access_count: 5, salience: 0.6, descriptors: "rrf,fusion,search",
            entities: vec![] },
        // --- More episodes ---
        TestEntry { id: 33, title: "Migrated from stdio to HTTP",
            content: "Today migrated all MCP servers from stdio to Streamable HTTP transport. Much cleaner.",
            memory_type: "episode", created_days_ago: 3.0, last_accessed_days_ago: Some(2.0),
            access_count: 2, salience: 0.5, descriptors: "mcp,migration,http",
            entities: vec![] },
        TestEntry { id: 34, title: "Added NLI contradiction check",
            content: "Yesterday added NLI contradiction detection to the retrieval pipeline. Uses cross-encoder/nli-MiniLM2.",
            memory_type: "episode", created_days_ago: 2.0, last_accessed_days_ago: Some(1.0),
        ...
        TestEntry { id: 35, title: "Tailscale VPN configuration",
            ...
            entities: vec![("Tailscale", "identifier")] },
        // --- Filler with specific search targets ---
        TestEntry { id: 36, title: "Tree-sitter code parsing",
            content: "Tree-sitter parses 13 languages. Incremental parsing. TAGS_QUERY for symbol extraction.",
            memory_type: "knowledge", created_days_ago: 16.0, last_accessed_days_ago: Some(8.0),
            access_count: 4, salience: 0.5, descriptors: "tree-sitter,parsing,code",
            entities: vec![] },
        TestEntry { id: 37, title: "Query expansion strategy",
            content: "Expand queries with stop-word removal, 2-token subsets, and quoted adjacent pairs. Max 6 variants.",
            memory_type: "knowledge", created_days_ago: 8.0, last_accessed_days_ago: Some(3.0),
            access_count: 3, salience: 0.5, descriptors: "query,expansion,search",
            entities: vec![] },
        TestEntry { id: 38, title: "Restic backup schedule",
            content: "Restic backups run nightly at 3am via systemd timer. R2 bucket. 7 daily + 4 weekly retention.",
            memory_type: "knowledge", created_days_ago: 25.0, last_accessed_days_ago: Some(10.0),
            access_count: 4, salience: 0.6, descriptors: "restic,backup,schedule",
            entities: vec![] },
        TestEntry { id: 39, title: "OAuth 2.1 for MCP servers",
            content: "All tunneled MCP servers use OAuth 2.1 with PKCE. DCR for dynamic client registration. Shared mcp-oauth module.",
            memory_type: "knowledge", created_days_ago: 10.0, last_accessed_days_ago: Some(2.0),
            access_count: 6, salience: 0.7, descriptors: "oauth,mcp,security",
            entities: vec![] },
        TestEntry { id: 40, title: "Hebbian association learning",
            ...
            memory_type: "knowledge", created_days_ago: 9.0, last_accessed_days_ago: Some(4.0),
            access_count: 3, salience: 0.6, descriptors: "hebbian,associations,learning",
            entities: vec![] },
    ]
}
...
fn ablation_configs() -> Vec<AblationConfig> {
    vec![
        AblationConfig { name: "Baseline (RRF only)", scorer: score_baseline },
        AblationConfig { name: "+Recency", scorer: score_recency_only },
        AblationConfig { name: "+Frequency", scorer: score_frequency_only },
        AblationConfig { name: "+Salience", scorer: score_salience_only },
        AblationConfig { name: "+Decay", scorer: score_decay_only },
        AblationConfig { name: "+All (full cognitive)", scorer: score_full },
    ]
}
...
fn setup_test_db() -> (TempDir, Store) {
    ...
    for entry in &corpus {
        ...
        // Insert entity edges
        for (value, kind) in &entry.entities {
            let entity_sql = format!(
                "INSERT OR IGNORE INTO graph (source_chunk, target_chunk, role, kind, symbol, strength) \
                 VALUES ({}, NULL, 'entity', '{}', '{}', 1.0)",
                entry.id, kind, value.replace('\'', "''"),
            );
            store.execute_batch(&entity_sql).unwrap();
        }
    }
...
fn run_ablation(store: &Store) -> Vec<(String, f64, f64, Vec<f64>)> {
    ...
    for config in &configs {
        ...
        for qt in &queries {
            ...
            let fts_hits = store.fts_search(qt.query, Some("memory"), 20).unwrap_or_default();
        ...
        results.push((config.name.to_string(), total_p5 / n, total_mrr / n, query_scores));
    }
...
fn bench_cognitive_component_isolation() {
    ...
    println!("  New (1 day):    {:.6} -> {:.6} ({:+.1}%)", base_score, score_recency_only(&new_hit),
             (score_recency_only(&new_hit) / base_score - 1.0) * 100.0);
    println!("  Old (60 days):  {:.6} -> {:.6} ({:+.1}%)", base_score, score_recency_only(&old_hit),
             (score_recency_only(&old_hit) / base_score - 1.0) * 100.0);

    // Test frequency: high vs low access
    let high_freq = make_hit(3, "FreqHi", base_score, "knowledge", 10.0, Some(1.0), 100, 0.5);
    let low_freq = make_hit(4, "FreqLo", base_score, "knowledge", 10.0, Some(1.0), 0, 0.5);

    println!("\n--- Frequency Boost ---");
    println!("  100 accesses:   {:.6} -> {:.6} ({:+.1}%)", base_score, score_frequency_only(&high_freq),
             (score_frequency_only(&high_freq) / base_score - 1.0) * 100.0);
    println!("  0 accesses:     {:.6} -> {:.6} ({:+.1}%)", base_score, score_frequency_only(&low_freq),
    ...
    println!("\n--- Salience Factor ---");
    println!("  Salience 1.0:   {:.6} -> {:.6} ({:+.1}%)", base_score, score_salience_only(&high_sal),
             (score_salience_only(&high_sal) / base_score - 1.0) * 100.0);
    println!("  Salience 0.0:   {:.6} -> {:.6} ({:+.1}%)", base_score, score_salience_only(&low_sal),
             (score_salience_only(&low_sal) / base_score - 1.0) * 100.0);

    // Test decay: episode vs identity
    let episode = make_hit(7, "Episode", base_score, "episode", 30.0, Some(20.0), 5, 0.5);
    let identity = make_hit(8, "Identity", base_score, "identity", 30.0, Some(20.0), 5, 0.5);
    let knowledge = make_hit(9, "Knowledge", base_score, "knowledge", 30.0, Some(20.0), 5, 0.5);

    println!("\n--- Decay Factor (20 days since last access) ---");
    println!("  Episode  (lambda=0.023): {:.6} -> {:.6} ({:+.1}%)", base_score, score_decay_only(&episode),
             (score_decay_only(&episode) / base_score - 1.0) * 100.0);
    println!("  Knowledge (lambda=0.005): {:.6} -> {:.6} ({:+.1}%)", base_score, score_decay_only(&knowledge),
             (score_decay_only(&knowledge) / base_score - 1.0) * 100.0);
    println!("  Identity (lambda=0.0):   {:.6} -> {:.6} ({:+.1}%)", base_score, score_decay_only(&identity),
    ...
    println!("  Best case:  {:.6} -> {:.6} ({:+.1}%)", base_score, score_full(&best_case),
             (score_full(&best_case) / base_score - 1.0) * 100.0);
    println!("  Worst case: {:.6} -> {:.6} ({:+.1}%)", base_score, score_full(&worst_case),
    ...
    assert!(score_full(&best_case) > score_full(&worst_case),
...
fn bench_cognitive_ablation() {
    ...
    // Print results table
    println!("{:<28} {:>10} {:>10} {:>12}", "Configuration", "Avg P@5", "Avg MRR", "vs Baseline");
    println!("{}", "-".repeat(64));
    ...
    for (name, avg_p5, avg_mrr, _) in &results {
        ...
        println!("{:<28} {:>10.3} {:>10.3} {:>12}", name, avg_p5, avg_mrr, delta);
    }
